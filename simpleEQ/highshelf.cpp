...
void Highshelf::Reset()
{
    ...
    b0_ = A*( (A+1.0f) + (A-1.0f)*tcos + 2.0f*sqA*alpha );
    b1_ = -2.0f*A*( (A-1.0f) + (A+1.0f)*tcos);
    b2_ = A*( (A+1.0f) + (A-1.0f)*tcos - 2.0f*sqA*alpha );
    a0_ = (A+1.0f) - (A-1.0f)*tcos + 2.0f*sqA*alpha;
    a1_ = 2.0f*( (A-1.0f) - (A+1.0f)*tcos  );
    a2_ = (A+1.0f) - (A-1.0f)*tcos - 2.0f*sqA*alpha;

}

void Highshelf::Init(float sample_rate)
{
...
float Highshelf::Process(float in)
{
    float xn, yn;
